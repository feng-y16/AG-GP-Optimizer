...
class STABLEOPT:
    def __init__(self,beta,init_num,mu0,epsilon,D,iter=100,init_point_option="function value weighted"):
        ...
        self.iter_initial_point=np.zeros(D)
        if init_point_option=="best point":
            self.init_point_option=1
            return
        if init_point_option=="random sample point":
            self.init_point_option=2
            return
        if init_point_option=="function value weighted":
    ...
    def run_onestep(self,iter):
        if self.t<self.T:
            ...
            print(fun_value)
            self.results[self.t-1]=fun_value
        else:
            print("t value error!")
            return 0
